...
function StructureMenu() {
    ...
    function effectMount() {
        submenuTodosRef.current &&
            submenuTodosRef.current.addEventListener("click", onClickTodos);
        submenuVistosRef.current &&
            submenuVistosRef.current.addEventListener("click", onClickVistos);
        submenuCapturadosRef.current &&
            submenuCapturadosRef.current.addEventListener(
                "click",
                onClickCapturados
            );
        submenuPokemonsRef.current &&
            submenuPokemonsRef.current.addEventListener(
                "click",
                onOpenPokemons
            );
        return () => {
            submenuTodosRef.current &&
                submenuTodosRef.current.removeEventListener(
                    "click",
                    onClickTodos
                );
            submenuVistosRef.current &&
                submenuVistosRef.current.removeEventListener(
                    "click",
                    onClickVistos
                );
            submenuCapturadosRef.current &&
                submenuCapturadosRef.current.removeEventListener(
                    "click",
                    onClickCapturados
                );
            submenuPokemonsRef.current &&
                submenuPokemonsRef.current.removeEventListener(
                    "click",
                    onOpenPokemons
                );
        };
    ...
    return (
        <hot-menu slot="menu">
            <div slot="product-name" className="productName">
                Pokémon
            </div>
            ...
            <hot-menu-item
                ref={submenuPokemonsRef}
                {...(history.location.pathname.includes("/pokemons") && {
                    open: "true",
                })}
            >
                ...
                <hot-menu-item
                    ref={submenuVistosRef}
                    class="submenu"
                    slot="collapsed"
                    {...(location.pathname === "/pokemons/vistos" && {
                        active: "true",
                    })}
                >
                    <span>Vistos</span>
            ...
            <Link to="/treinadores">
                <hot-menu-item
                    {...(history.location.pathname === "/treinadores" && {
                        active: "true",
                    })}
                >
                    <div className="hot-application-menu__icon icon-label--menu-style">
